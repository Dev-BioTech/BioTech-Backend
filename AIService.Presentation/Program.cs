...
if (string.IsNullOrEmpty(connectionString))
{
    ...
    {
        var dbHost = Environment.GetEnvironmentVariable("AI_DB_HOST") ?? Environment.GetEnvironmentVariable("DB_HOST");
        if (!string.IsNullOrEmpty(dbHost))
        {
            var dbPort = Environment.GetEnvironmentVariable("AI_DB_PORT") ?? Environment.GetEnvironmentVariable("DB_PORT") ?? "5432";
            var dbName = Environment.GetEnvironmentVariable("AI_DB_NAME") ?? Environment.GetEnvironmentVariable("DB_DATABASE") ?? Environment.GetEnvironmentVariable("DB_NAME") ?? "biotech_db";
            var dbUser = Environment.GetEnvironmentVariable("AI_DB_USER") ?? Environment.GetEnvironmentVariable("DB_USER");
            var dbPassword = Environment.GetEnvironmentVariable("AI_DB_PASSWORD") ?? Environment.GetEnvironmentVariable("DB_PASSWORD");
